...
import { ProjectLogs as ProjectLogsType } from 'types'
...
export function ProjectLogs({ projectId }: { projectId: string }) {
	...
	const [logs, setLogs] = React.useState<ProjectLogsType>()

	React.useEffect(() => {
		const fetchLogs = async () => {
			setLoading(true)
			const res = await api.logsRouter.getProjectLogs.query(projectId)
			setLogs(res)
			setLoading(false)
		}
		fetchLogs()
